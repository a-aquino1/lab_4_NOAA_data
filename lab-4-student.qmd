---
title: "Lab 4: Iporting NOAA Buoy Data from Monterey Bay"
...
```{r}
#| label: load-data
#download.file("https://jcanner.github.io/stat_210_2025_website/docs/labs/student/data/buoy_2024.txt", destfile = "buoy_2023.txt")

buoy_data <- read_table("data-raw/buoy_2023.txt")
## Downloaded the data file by looking at the 'Help' panel then commented it out and loaded the data into the data-raw file
```
...
buoy_data <- read_table("data-raw/buoy_2023.txt", comment = "#")
## Referred to the Rstudio book to 'drop all lines that start with #'. Updated the table
```
...
library(janitor)
buoy_data <- read_table("data-raw/buoy_2023.txt", comment = "#")
clean_names(buoy_data)%>%
  rename(mm = mm, 
         mm_2 = mm_2)
## loaded the janitor package and clean_name in order to rename mm into separate distinctions
```
...
write_csv(buoy_data, "data-raw/buoy_update.csv")
              
   
## wrote the .txt data file into a .csv 
```
...
#| eval: FALSE
cleaned_data <- read_csv("data-raw/buoy_update.csv", 
         col_types = cols(.default = col_character()), 
         na = c("99", "999"))


## cleaned the data and changed the data into character, looked in the book
...
library(viridis)
cleaned_data <- read_csv("data-clean/buoy_update.csv")

ggplot(data = cleaned_data, 
       aes(x = MM, 
           y = ATMP, 
           color = factor(MM))) +
  geom_point(position = "jitter", show.legend = FALSE, alpha = 0.3) +
  geom_smooth(se = FALSE, color = "black", show.legend = FALSE) +
  labs(title = "Monterey Canyon Air Temperature in 2024", 
       x = "Months in 2024", 
  ...
  theme_minimal()
##used the book, rstudio book, rstudio archive website, and rcharts for references ("https://r-charts.com/base-r/axes/#axis-limits") (https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html)
```
...
#The Simpsons data set is a collection of character dialog, supporting character reacurrance, as well as talk about declining TV ratings. The data is an analysis of, what I belive, to be the overall result of episode quality and script as the seasons pregressed.
https://toddwschneider.com/posts/the-simpsons-by-the-data/
Questions:
How has the dialog among the main cast of chracters changed over the years?
How does The Simpsons dialong compare to other mature carttons at the time?
How do curent events affect the dialong and the chracter reacurece in each season or episodes.
